...
\begin{flashcard}[Algorithmus]{Konstruktion kubischer Splines}
	\begin{enumerate}
		...
		\item $i$-tes Polynom gegeben durch $s_i(x) = y_{i-1} + (x - x_{i-1}) y_{i-1,i} + (x - x_{i-1})(x - x_i)[\alpha_i(x - x_{i-1}) + \beta_i (x - x_i)]$
	\end{enumerate}
\end{flashcard}
...
\begin{flashcard}[LGS]{LGS für periodische Splines}
	$$
		\tilde{d} = \begin{pmatrix}
			0\\
			d_1\\
